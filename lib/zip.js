			...
			if (i < len) {
				try {
					...
					if (visitor(entry, i, len) !== false) {
						if (!ignores || !ignores.test(entry.entryName)) {
							if (entry.isDirectory) {

								var thePath = afs.resolvePath(dest, entry.entryName.toString());
								// afs.unlinkDirSyncRecursive(thePath);
								Utils.makeDir(thePath);
							} else {
								...
								Utils.writeFileTo(
									...
									content,
									overwrite, (entry.header.attr && (entry.header.attr >>> 16) || defaultPerm) & 0777
								);
							}
						}
					}
...
exports.readAsText = function(file, path) {
	return new AdmZip(file).readAsText(path);
}